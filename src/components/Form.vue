...
const submitForm = () => {
    ...
    validateGender(true);
    if (!errors.value.username && !errors.value.password && !errors.value.resident && !errors.value.gender
        && !errors.value.reason) {
...
const validateReason = (blur) => {
    if (formData.value.reason.length < 5) {
        if (blur) errors.value.reason = "Reason must be at least 5 characters long.";
    } else {
